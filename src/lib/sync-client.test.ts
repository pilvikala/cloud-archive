...
describe("SyncClient", () => {
  ...
  beforeEach(async () => {
    ...
    mockGcpClient = new GcpClient("test-bucket") as jest.Mocked<GcpClient>;
    mockGcpClient.uploadFile.mockResolvedValue(undefined);
    mockGcpClient.listContent.mockResolvedValue([]);

    syncClient = new SyncClient(tempDir, mockGcpClient);
  });
  ...
  it("should upload all files when none exist in bucket", async () => {
    ...
    expect(result.uploadedFiles).toContain("file2.txt");
    expect(result.uploadedFiles).toContain("subdir/file3.txt");
  ...
  it("should skip existing files with same size", async () => {
    ...
    mockGcpClient.listContent.mockResolvedValueOnce([
      { name: "file1.txt", size: file1Stats.size },
      { name: "subdir/file3.txt", size: file3Stats.size },
    ]);
    ...
    // Verify uploadFile was called only once
  ...
  it("should upload files with different sizes", async () => {
    ...
    const file1Stats = await fs.promises.stat(path.join(tempDir, "file1.txt"));
    mockGcpClient.listContent.mockResolvedValueOnce([
      { name: "file1.txt", size: file1Stats.size + 1 }, // Different size
    ]);
